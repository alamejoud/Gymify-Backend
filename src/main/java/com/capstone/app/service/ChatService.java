...
public class ChatService implements ChatServiceInterface{
    ...
    public List<UserEntity> getContacts(String token, String role, String search) {
        ...
        contacts.forEach(contact -> {
            contact.setUnreadMessages((int) contact.getSentMessages().stream().filter(
                    message -> message.getMessageTo().getUsername().toLowerCase().equals(
                            jwtService.extractUsername(token).toLowerCase()) && message.getMessageStatus().equals("sent")).count());
        });
        contacts.forEach(contact -> {
            MessageEntity lastSentMessage = contact.getSentMessages().stream().filter(
                    message -> message.getMessageTo().getUsername().toLowerCase().equals(
                            jwtService.extractUsername(token).toLowerCase())).max((m1, m2) -> m1.getMessageDate().compareTo(m2.getMessageDate())).orElse(null);
            MessageEntity lastReceivedMessage = contact.getReceivedMessages().stream().filter(
                    message -> message.getMessageFrom().getUsername().toLowerCase().equals(
                            jwtService.extractUsername(token).toLowerCase())).max((m1, m2) -> m1.getMessageDate().compareTo(m2.getMessageDate())).orElse(null);
